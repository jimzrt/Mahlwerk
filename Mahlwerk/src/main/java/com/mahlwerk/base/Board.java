...
public class Board {
		...
		for (int i = 0; i < SIZE; i++) {
			for (int j = 0; j < SIZE; j++){
				if(validPositions[i+j*SIZE])
					...
					pieces[i+j*SIZE] = -1;
				//pieces.add(new Piece(Piece.PieceColor.EMPTY, j, i));
	...
	public PieceColor getPieceColor(int x, int y){
		...
		case 0: return PieceColor.EMPTY;
		case 1: return PieceColor.WHITE;
		case 2: return PieceColor.BLACK;
		}
		return null;
	}
...
private void buildMillMap() {
	...
	public synchronized void makeMove(Move move){
		...
		//	updateKey(move.removePiece.x, move.removePiece.y, PieceColor.EMPTY);
	...
	public synchronized void setPieceColor(Piece piece, PieceColor color) {
		...
		switch(color){
		case EMPTY: pieces[piece.x + piece.y * SIZE] = 0;return;
		case WHITE: pieces[piece.x + piece.y * SIZE] = 1;return;
		case BLACK: pieces[piece.x + piece.y * SIZE] = 2;return;
		}
	}
	...
	public synchronized int getPiecesByColorCount(PieceColor color) {
	 ...
	 public int[] getArrayIndex(int[] arr,int value) {
		 ...
		 int[] indices = IntStream.range(0, arr.length)
...
//	    return index;
		...
		public synchronized boolean endReached() {
			...
			return false;
		//	return !canMove(PieceColor.BLACK, false) && !canMove(PieceColor.WHITE, false);
		}
